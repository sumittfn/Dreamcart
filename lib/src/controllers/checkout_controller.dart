...
class CheckoutController extends ControllerMVC {
  ...
  void addOrder(List<Cart> carts) async {
    ...
    _order.deliveryFee = payment.method == 'Take Away'
        ? "0.0"
        : deliveryFee.toString();
    _order.user = userRepo.currentUser.value;
    OrderStatus _orderStatus = new OrderStatus();
    _orderStatus.id = '1'; // TODO default order status Id
    _order.orderStatus = _orderStatus;
    _order.deliveryAddress = address;
    _order.discount = discount.toString();
    _order.promocode = promocode;
    _order.delivery_address_id = settingRepo.deliveryAddress.value.id;
    carts.forEach((_cart) {
